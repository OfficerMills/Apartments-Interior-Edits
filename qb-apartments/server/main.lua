...
RegisterNetEvent('apartments:server:openStash', function(CurrentApartment)
    ...
    if not Player then
        print("^1[Error] Player not found for source: ^0" .. tostring(src))
        return
    end

    if not CurrentApartment or CurrentApartment == "" then
        print("^1[Error] Invalid apartment stash identifier: ^0" .. tostring(CurrentApartment))
        return
    end

    print("Opening stash for player:", Player.PlayerData.citizenid, "Apartment:", CurrentApartment)
    -- FIX: Pass `src` as the origin parameter to `OpenInventory`
    exports['qb-inventory']:OpenInventory('stash', CurrentApartment, nil, src)
end)
