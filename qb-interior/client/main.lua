...
local function CreateShell(spawn, exitXYZH, model)
    print("Debug: spawn = ", json.encode(spawn))
    print("Debug: exitXYZH = ", json.encode(exitXYZH))
    ...
    if not house then
        print("Error: Failed to create object for model:", model)
        return
    end
...
exports('CreateApartmentFurnished', function(spawn)
    ...
    -- Offsets provided
    POIOffsets.exit = json.decode('{"x": 1.464355, "y": 0.649414, "z": 0.009995, "h": 6.190939}') --
    POIOffsets.clothes = json.decode('{"x": 3.125000, "y": 6.716797, "z": 0.010017, "h": 134.299973}') --
    POIOffsets.stash = json.decode('{"x": -1.0001566, "y": 7.894629, "z": 0.010017, "h": 101.115784}') --
    POIOffsets.logout = json.decode('{"x": -3.1802972, "y": -1.999250, "z": 0.009987, "h": 359.190338}')

    local spawnPointX = 1.336426 
    local spawnPointY = 0.861633 
    local spawnPointZ = 0.010021
    local spawnPointH = 190.015076 
    ...
    RequestModel(`t2_furn_shell`) -- Use the appropriate MLO model name
    while not HasModelLoaded(`t2_furn_shell`) do
        Wait(3)
    end
  
    local house = CreateObject(`t2_furn_shell`, spawn.x, spawn.y, spawn.z, false, false, false)
    FreezeEntityPosition(house, true)
    objects[#objects + 1] = house
    TeleportToInterior(spawn.x + spawnPointX, spawn.y + spawnPointY, spawn.z + spawnPointZ, spawnPointH)

    -- Handle first character creation for new players
    if IsNew then
